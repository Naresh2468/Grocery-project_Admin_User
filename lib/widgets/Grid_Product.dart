...
class GridProductWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      //make sure the collection is proper same as the firestoreCloud
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (snapshot.data!.docs.isNotEmpty) {
            return snapshot.data!.docs.length == 0 //***additional changes***
                ? Center(
                    child: TextWidget(
                        text: "You didn't add any  product ", color: color),
                  )
                : GridView.builder(
